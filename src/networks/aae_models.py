...
class encoder_rgb(nn.Module):
    def __init__(self, object_total_n, capacity=1, code_dim=128):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(3 + self.object_total_n, 128 *
                      capacity, 5, stride=2, padding=2),
            nn.ReLU(),
        )
        self.layer2 = nn.Sequential(
            nn.Conv2d(128 * capacity, 256 * capacity, 5, stride=2, padding=2),
...
class decoder_depth(nn.Module):
    def __init__(self, capacity=1, code_dim=128):
        ...
        self.layer1 = nn.Sequential(
            nn.ConvTranspose2d(256 * capacity, 128 * capacity,
                               5, 2, padding=2, output_padding=1),
            nn.ReLU()
        )
        self.layer2 = nn.Sequential(
            nn.ConvTranspose2d(128 * capacity, 128 * capacity,
                               5, 2, padding=2, output_padding=1),
            nn.ReLU()
        )
        self.layer3 = nn.Sequential(
            nn.ConvTranspose2d(128 * capacity, 64 * capacity,
                               5, 2, padding=2, output_padding=1),
            nn.ReLU()
        )

        self.layer4 = nn.Sequential(
            nn.ConvTranspose2d(64 * capacity, 1, 5, 2,
...
class decoder_rgb(nn.Module):
    def __init__(self, capacity=1, code_dim=128):
        ...
        self.layer1 = nn.Sequential(
            nn.ConvTranspose2d(512 * capacity, 256 * capacity,
                               5, 2, padding=2, output_padding=1),
            nn.ReLU()
        )
        self.layer2 = nn.Sequential(
            nn.ConvTranspose2d(256 * capacity, 256 * capacity,
                               5, 2, padding=2, output_padding=1),
            nn.ReLU()
        )
        self.layer3 = nn.Sequential(
        ...
        self.layer4 = nn.Sequential(
            nn.ConvTranspose2d(128 * capacity, 3, 5, 2,
                               padding=2, output_padding=1)
        )
...
class AAE(nn.Module):
    def __init__(self, object_names, modality='rgbd', capacity=1, code_dim=128, model_path=None):
        ...
        if self.modality == 'rgbd':
            ...
            self.depth_decoder = decoder_depth(
                capacity=capacity, code_dim=code_dim)
            ...
            self.encoder = encoder_rgb(
                object_total_n=1, capacity=capacity, code_dim=code_dim)
            self.decoder = decoder_rgb(capacity=capacity, code_dim=code_dim)
            self.encoder.apply(weights_init)
            self.decoder.apply(weights_init)
        ...
        bootstrap_const = 2000
        self.B_loss = BootstrapedMSEloss(bootstrap_const)
    ...
    def compute_codebook(self, code_dataset, save_dir, batch_sz=500, code_dim=128, save=True):
        ...
        for inputs in code_generator:
            ...
            class_info = torch.ones((rgb.size(0), 1, rgb.size(
                2), rgb.size(3)), dtype=torch.float32).float().cuda()
            input = torch.cat((rgb, class_info), dim=1)
            code = self.encoder.forward(input).detach().view(input.size(0), -1)

            print(code.size())

            codebook_cpt[step * codebook_batch_size:step *
                         codebook_batch_size + code.size(0), :] = code
            codepose_cpt[step * codebook_batch_size:step *
                         codebook_batch_size + code.size(0), :] = poses.squeeze(1)
    ...
    def compute_codebook_rgbd(self, code_dataset, save_dir, batch_sz=250, code_dim=128, save=True):
        ...
        for inputs in code_generator:
            ...
            codebook_depth_cpt[step * codebook_batch_size:step *
                               codebook_batch_size + code.size(0), :] = code_depth
            codebook_cpt[step * codebook_batch_size:step *
                         codebook_batch_size + code.size(0), :] = code
            codepose_cpt[step * codebook_batch_size:step *
                         codebook_batch_size + code.size(0), :] = poses.squeeze(1)
        ...
        if save:
            torch.save((codebook_cpt, codepose_cpt,
                        codebook_depth_cpt), save_dir)
            print('code book is saved to {}'.format(save_dir))
