...
result_dir = horzcat(root_dir, '/experiments/ycbv/inference/010_potted_meat_can_16k/batch_1');
test_cls_idx = 9; % 1-indexed (e.g. 2 = cracker box, 3 = sugar, 9 = meat can)
...
experiments = experiments(3:end); % exclude . and ..
for e = 1:numel(experiments)
    extract_id = strsplit(experiments(e).name, '_');
    experiments(e).identifier = extract_id{1}; % Hybrid, Inlier, PoseEval, baseline, ycb
    experiment_dir = horzcat(experiments(e).folder, '/', experiments(e).name);
    all_seqs = dir(horzcat(experiment_dir, '/*.txt'));
    ...
    experiments(e).all_seqs = all_seqs;
end
...
for i = 1:numel(keyframes)
    ...
    for j = 1:numel(gt.cls_indexes) % 1-indexed (see plot_accuracy_keyframe.m/L32
        if gt.cls_indexes(j) == test_cls_idx
            ...
            for e = 1:numel(experiments)
                index = experimentId2index(experiments(e).identifier);
                all_seqs = experiments(e).all_seqs;
                ...
                if all_seqs(s).miss_detections(frame_id)
                    ...
                    distances_sys(count, index) = inf;
                    distances_non(count, index) = inf;
                    errors_rotation(count, index) = inf;
                    errors_translation(count, index) = inf;
                else
                    % get poses
                    RT_gt = gt.poses(:,:,j);
                    RT = zeros(3, 4);
                    ...
                    % evaluate
                    distances_sys(count, index) = adi(RT, RT_gt, models{cls_index}');
                    distances_non(count, index) = add(RT, RT_gt, models{cls_index}');
                    errors_rotation(count, index) = re(RT(1:3, 1:3), RT_gt(1:3, 1:3));
                    errors_translation(count, index) = te(RT(:, 4), RT_gt(:, 4));
                end
            end
        end
    end
...
function index = experimentId2index(exp_id)
if strcmp(exp_id, 'baseline')
    index = 1;
elseif strcmp(exp_id, 'Hybrid')
    index = 2;
elseif strcmp(exp_id, 'Inlier')
    index = 3;
elseif strcmp(exp_id, 'PoseEval')
    index = 4;
elseif strcmp(exp_id, 'ycb')
    index = 5;
end
end
