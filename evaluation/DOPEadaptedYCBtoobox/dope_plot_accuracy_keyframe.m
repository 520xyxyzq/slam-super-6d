...
for k = 1:numel(classes)
    ...
    if isempty(index)
        index = 1:size(distances_sys,1); % include all if cls not found
    end
...
mrec=[0 ; rec ; 0.1]; % insert 0 in the beginning and 0.1 at end
mpre=[0 ; prec ; prec(end)];
for i = 2:numel(mpre)
    mpre(i) = max(mpre(i), mpre(i-1)); % ensure mono-increasing
end
i = find(mrec(2:end) ~= mrec(1:end-1)) + 1; % find where two successive elements are ~=
ap = sum((mrec(i) - mrec(i-1)) .* mpre(i)) * 10; % * 10 since max(mrec) = 0.1 to make area = 1
