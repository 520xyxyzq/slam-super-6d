...
for i = 1:numel(keyframes)
    ...
    % load PoseCNN result
    filename = sprintf('results_PoseCNN/%04d.mat', i - 1); % 0-indexed
    ...
    % for each gt poses
    for j = 1:numel(gt.cls_indexes) % indices are in [1,21]
        count = count + 1;
        cls_index = gt.cls_indexes(j);
        RT_gt = gt.poses(:, :, j); % get the object pose of object cls_index
        ...
        % network result
        roi_index = find(result.rois(:, 2) == cls_index); % match to the object label
        if isempty(roi_index) == 0 % not empty
            RT = zeros(3, 4);

            % pose from network
            RT(1:3, 1:3) = quat2rotm(result.poses(roi_index, 1:4)); % use roi_index to find pose prediction
        ...
        else % if the cls_index not found in predictions
